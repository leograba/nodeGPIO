...
$(function main(){ //wait for the page to be fully loaded
	...
	timerHandler = setInterval(swAction, 5000); //updates the GPIO Status every 0,2s
	
	$(".true_btn").click(whichBtn); //if element of class "true_btn" is clicked
});
...
function updateCurrentStatus(){
	...
	getGPIO(function update(gpio_status){
		for (next_pin in gpio_status){ //iterate through all GPIO
			if(next_pin.indexOf("SW") != -1){ //if it is a switch
				gpio_status[next_pin] = !gpio_status[next_pin]; //then inverts the logic, for 0 means pressed
			}
			console.log(next_pin + " ---> " + gpio_status[next_pin]);
			if(gpio_status[next_pin]){ //if state is ON (or HIGH)
...
function swAction(){
	...
	getGPIO(function doAction(gpio_current){ //get GPIO status from server
		...
		for (next_pin in gpio_current){ //iterate through all GPIO
			if(!gpio_current[next_pin]){ //if current GPIO was pressed
				...
				case "SW2": //when pushbutton 2 is pressed
					changeLedState(curr_led, (1 - gpio_current[curr_led])); //invert currently selected LED state
					break;//this should be removed if button priority is not desired
